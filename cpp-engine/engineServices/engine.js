...
function runStockfish(fen) {
  return new Promise((resolve, reject) => {
    const stockfishPath = path.resolve(__dirname, '../../cpp-engine/stockfish.exe');
    console.log(`[DEBUG] Launching Stockfish: ${stockfishPath}`);
    ...
    engine.stdout.on('data', (data) => {
      ...
      // UCI Protocol - ready to send commands
      if (!uciOkReceived && chunk.includes('uciok')) {
        uciOkReceived = true;
        engine.stdin.write('isready\n');
        console.log('[DEBUG] Sent: isready');
      ...
      // If ready, set position and request a move at a certain depth
      else if (uciOkReceived && !readyOkReceived && chunk.includes('readyok')) {
        readyOkReceived = true;
        engine.stdin.write(`position fen ${fen}\n`);
        engine.stdin.write('go depth 20\n');  // Increase depth for more accuracy
        console.log('[DEBUG] Sent: position fen', fen);
        console.log('[DEBUG] Sent: go depth 20');
      } 
      
      // If bestmove is found, resolve the output
      else if (readyOkReceived && chunk.includes('bestmove')) {
        console.log('[DEBUG] Found bestmove!');
        engine.stdin.end();
        resolve(output);
...
// Parse Stockfish output to extract bestmove, score, depth, pv, nodes, and other info
function parseStockfishOutput(output) {
  const result = {};
  const lines = output.split('\n');
  let depth = 0;
  ...
  for (const line of lines) {
    ...
    // Get nodes searched by the engine
    if (line.includes('nodes')) {
      const nodesMatch = line.match(/nodes (\d+)/);
      if (nodesMatch) nodes = parseInt(nodesMatch[1]);
    }

    // Get the time taken by the engine to calculate
